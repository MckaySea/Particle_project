...
Game::Game() : world(20, 20) {
  ...
  // keeping lines for our ui, was haivng problems with space and flickering
  world = World(r - 5, c);
  frame = 0;
  frame_count = 0;
}
...
void Game::run() {
  ...
  while (is_running) {
    ...
    // buildin the entire frame buffer to avoid flickering from unbuffered cerr
...
void Game::render() {
  // Rendering logic relocated to Game::run stringstream buffer to prevent
  // unbuffered cerr terminal flickering. Look inside Game::run()
}
